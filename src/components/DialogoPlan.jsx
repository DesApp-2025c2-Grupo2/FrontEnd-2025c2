...
export default function DialogoPlan({ abierto, valorInicial, onCerrar, onGuardar }) {
  const [form, setForm] = useState({ codigo: '', nombre: '', descripcion: '', precio: 0, activo: true });
  ...
  const errores = useMemo(() => {
    ...
    const duplicado = planes?.some(p => p.id !== form.id && (String(p.codigo).trim().toLowerCase() === codigoStr || String(p.nombre).trim().toLowerCase() === nombreStr));
    if (duplicado) {
  ...
  return (
    <Dialog open={abierto} onClose={onCerrar} fullWidth maxWidth="sm">
      <DialogTitle sx={{ fontWeight: 800, color: '#111827' }}>{form.id ? 'Editar plan' : 'Agregar plan'}</DialogTitle>
      <DialogContent
        dividers
        sx={{
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingTop: 8 }}>
          <TextField label="Código" value={form.codigo} onChange={cambiar('codigo')} type="number" fullWidth error={!!errores.codigo} helperText={errores.codigo} InputLabelProps={{ sx: { fontWeight: 700, color: '#111827' } }} inputProps={{ sx: { color: '#111827', fontWeight: 600 } }} />
          ...
          <TextField label="Precio" value={form.precio} onChange={cambiar('precio')} type="number" fullWidth InputLabelProps={{ sx: { fontWeight: 700, color: '#111827' } }} inputProps={{ sx: { color: '#111827', fontWeight: 600 } }} />
          <FormControlLabel control={<Switch checked={!!form.activo} onChange={cambiar('activo')} />} label="Activo" />
        </div>
      </DialogContent>
      <DialogActions>
